...
final tagsPorRestauranteProvider = StreamProvider<Map<String, List<Tag>>>((
  ref,
) {
...
final restaurantesResumenProvider =
    Provider<AsyncValue<List<RestauranteResumen>>>((ref) {
      ...
      if (restaurantesAsync.isLoading ||
          tagsAsync.isLoading ||
          platosAsync.isLoading) {
        return const AsyncValue.loading();
      }
      final error =
          restaurantesAsync.error ?? tagsAsync.error ?? platosAsync.error;
      if (error != null) {
      ...
      final resumen = restaurantes.map((restaurante) {
        final platosDelRestaurante =
            platosPorRestaurante[restaurante.id] ?? const [];
